...
Proposal payloads, sensitive read request payloads, and audit metadata use the
same recursive sensitive-data redactor before persistence. Secret-shaped keys
such as authorization headers, cookies, API keys, application passwords,
private keys, and tokens are stored as `[redacted]`; obvious inline bearer,
cookie, token, API-key, and password strings are also redacted. Non-secret Core
...
- Lifecycle status transitions for proposals and sensitive read requests must
  include the expected current status in the SQL `WHERE` clause. A stale
...
Plan-to-proposal intake is bounded before proposal creation. Core rejects
payloads larger than its plan intake byte limit, rejects more than 25
`write_actions` in a single plan, and applies narrower action caps to specific
batch review shapes such as media optimization and block theme site plans.
